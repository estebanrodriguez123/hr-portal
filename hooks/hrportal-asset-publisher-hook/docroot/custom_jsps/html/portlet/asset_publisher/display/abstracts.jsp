...
/* [RIVET, CUSTOMIZATION, BEGIN] */
viewFullContentURL.setWindowState(WindowState.MAXIMIZED);
viewFullContentURL.setParameter("backUrl", PortalUtil.getCurrentURL(renderRequest));
viewFullContentURL.setParameter("hideHeader", "true");
viewFullContentURL.setParameter("hideMetadata", "true");
...
<c:if test="<%= show %>">
	<div class="asset-abstract <%= defaultAssetPublisher ? "default-asset-publisher" : StringPool.BLANK %>">
		...
		<%		
			String iconURL = "";
			List<AssetCategory> categories = AssetCategoryLocalServiceUtil.getCategories(assetEntry.getClassName(), assetEntry.getClassPK());
			for (AssetCategory category: categories) {
				if (category.getName().equals("carousel")) {
					continue;
				}
				long folderId = DLFolderLocalServiceUtil.getFolder(themeDisplay.getScopeGroupId(), 0, "asset publisher icons").getFolderId();
				DLFileEntry iconDLFileEntry = DLFileEntryLocalServiceUtil.getFileEntry(themeDisplay.getScopeGroupId(), folderId, category.getName());
				FileEntry iconFileEntry = DLAppLocalServiceUtil.getFileEntry(iconDLFileEntry.getFileEntryId());
				iconURL = DLUtil.getPreviewURL(iconFileEntry, iconFileEntry.getFileVersion(), themeDisplay,"");
				iconURL = Validator.isNotNull(iconURL) ? iconURL : assetRenderer.getIconPath(renderRequest);
			}
		%>
		<%-- END Rivet Customization --%>
