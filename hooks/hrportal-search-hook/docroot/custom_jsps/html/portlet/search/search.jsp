...
/** RIVET LOGIC UPDATE CODE **/
List<Group> sites =  user.getMySites();
Group group = themeDisplay.getScopeGroup();
/** END RIVET LOGIC UPDATE CODE **/
...
<aui:form action="<%= searchURL %>" method="get" name="fm" onSubmit="event.preventDefault();">
	...
	<aui:fieldset id="searchContainer">
		...
		<aui:select inlineField="<%= true %>" label="" name="groupId2">
	...
	A.on(
		...
		function(event) {
			...
			if (targetId === '<portlet:namespace />search') {
				var combo = A.one('#<portlet:namespace />groupId2').get('value');
				A.one('#<portlet:namespace />groupId').set('value', combo);
			}
