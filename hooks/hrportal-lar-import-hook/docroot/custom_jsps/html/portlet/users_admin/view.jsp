...
<aui:script>
	function <portlet:namespace />importUserPages() {
    	document.<portlet:namespace />fm.method = "post";
		document.<portlet:namespace />fm.encoding = "multipart/form-data";
		document.<portlet:namespace />fm.<portlet:namespace /><%=Constants.CMD%>.value = "import";
		document.<portlet:namespace />fm.<portlet:namespace />struts_action.value = "/users_admin/edit_user";
...
<aui:form action="<%= portletURLString %>" method="post" name="fm">
	...
	<aui:input name="struts_action" type="hidden" value="/users_admin/view_users" />
...
<% /* [RIVET, CUSTOMIZATION, BEGIN] */ %>
	<c:if test="<%=permissionChecker.isOmniadmin() %>">
		<div class="portlet-msg-info" style="background-image:none;">
			<div>
				<label class="aui-field-label">
					<liferay-ui:message key='<%= "rivet-import-a-lar-file-to-overwrite-user-public-pages-msg" %>' />
				</label>
				<input name="<portlet:namespace />importPublicPagesFileName" size="50" type="file" />
			</div>
			<br />
			<div>
				<label class="aui-field-label">
					<liferay-ui:message key='<%= "rivet-import-a-lar-file-to-overwrite-user-private-pages-msg" %>' />
				</label>
				<input name="<portlet:namespace />importPrivatePagesFileName" size="50" type="file" />
			</div>
			<br />
			<input type="button" value="<liferay-ui:message key="import" />" onClick="<portlet:namespace />importUserPages();">
		</div>
	</c:if>
<% /* [RIVET, CUSTOMIZATION, END] */ %>
</aui:form>
